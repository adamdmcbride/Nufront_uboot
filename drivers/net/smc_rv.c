...
/*
 * Functions required by u-boot struct eth_device
 */
static void smsc9118_halt(struct eth_device* dev);
static int  smsc9118__rx(struct eth_device * dev);
static int  smsc9118_send(struct eth_device* dev, volatile void *packet, int length);
static void smc_halt(struct eth_device* dev);
static int  smc_rx(struct eth_device* dev);
static int  smc_send(struct eth_device * dev, volatile void *packet, int length);
...
static char unsigned smc_mac_addr[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};

static int smc_device_initialise(bd_t *bd, struct eth_device *dev)
{
	int retval = 0;
	int i;
	/*  Get MAC address */
	/* ASSUMING we can do this BEFORE the ethernet hware is initialised in any way */
	retval = smc_get_ethaddr (bd);	/* set smc_mac_addr, and sync it with u-boot globals */
	if (retval < 0)
		return -1;
...
# endif
	...
	dev->recv = smc_rx;
	/* ASSUMES device 0 */
	sprintf(dev->name, "%s-%hu", "SMC_RV-SMC", 0);
	return 0;
}
...
#endif /* CONFIG_NET_MULTI  - else moved to board initialization routine */
 ...
 */
int smc9118_set_valid_ethaddr(unsigned int *mac_low, unsigned int *mac_high)
{
	uchar mac[6];
 ...
 */
static int smsc9118_initialise(struct eth_device *dev, bd_t *bd)
{
	int error = 1;
	ulong start;
		...
		*SMSC9118_AFC_CFG = 0x006E3740;
		...
		if(smsc9118_wait_eeprom()) {
			...
			*SMSC9118_GPIO_CFG = 0x70070000;
			...
			// else check valid
// Before NET_MULTI - now set by board intialisation if(smc9118_set_valid_ethaddr()){
...
int smsc9118_device_initialise(bd_t* bd, struct eth_device *dev)
{
	/*  Get MAC address */
	unsigned long addrl, addrh;
	/* Check the cast will be OK */
	if(sizeof(unsigned long) != sizeof(unsigned int))
		printf("smc_rv.c::smsc9118_device_initialise() - PANIC!! get variable lengths to match\n");
	smc9118_set_valid_ethaddr((unsigned int*)&addrl, (unsigned int*)&addrh);
	...
	dev->init = smsc9118_initialise;
	dev->halt = smsc9118_halt;
	dev->send = smsc9118_send;
	dev->recv = smsc9118__rx;
	/* ASSUMES device 0 */
	sprintf(dev->name, "%s-%hu", "SMC_RV-SMSC9118", 0);
	return 0;
}
...
/* The board initialise routine */
int smc_rv_initialize(bd_t * bd, int base_addr)
{
	/* This will do for both drivers */
	...
	switch (chip_type){
	case LAN91C111:	
		retval = smc_device_initialise(bd, dev);
		break;

	case LAN9118:	
		retval = smsc9118_device_initialise(bd, dev);
		break;
	}
...
int smc_rx(struct eth_device* dev) {
       return smc_rcv();
}

int smc_send(struct eth_device * dev, volatile void *packet, int length) {
