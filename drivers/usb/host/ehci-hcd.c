		 ...
		 int length, struct devrequest *req)
{
	...
	case USB_REQ_GET_STATUS | ((USB_DIR_IN | USB_RT_HUB) << 8):
		debug("USB_REQ_GET_STATUS (DIR_IN|RT_HUB)\n");
...
CLR_PORT_RESET:
		...
		debug("USB_REQ_GET_STATUS (DIR_IN|RT_PORT):regval 0x%x\n",reg);
		if (reg & EHCI_PS_CS)
		...
		if (reg & EHCI_PS_PR &&
			...
			debug("force port reset to complete\n");
			reg = reg & ~(EHCI_PS_PR | EHCI_PS_CLEAR);
			ehci_writel(status_reg, reg);
			ret = handshake(status_reg, EHCI_PS_PR, 0, 2 * 1000);
			debug("force port reset completed:regval 0x%x\n",ehci_readl(status_reg));
			goto CLR_PORT_RESET;
			if (!ret)
	...
	case USB_REQ_SET_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		reg = ehci_readl(status_reg);
		debug("USB_REQ_SET_FEATURE (DIR_OUT|RT_PORT):regval:0x%x\n",reg);
		reg &= ~EHCI_PS_CLEAR;
	...
	case USB_REQ_CLEAR_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		reg = ehci_readl(status_reg);
		debug("USB_REQ_CLEAR_FEATURE (DIR_OUT|RT_PORT):regval:0x%x\n",reg);
		switch (le16_to_cpu(req->value)) {
		case USB_PORT_FEAT_ENABLE:
			reg &= ~EHCI_PS_PE;
			break;
