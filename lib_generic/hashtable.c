...
int hread_buf(struct hsearch_data *htab, const char *env, size_t size,const  char sep,struct bootsel *bootsel)
{
	char *data,*sp, *dp,*name, *value;
...
//			printf("*sp++ = %c\n",*sp);
		...
		*sp++ = '\0';
		++dp;
		if(strncmp(name,"Boot system", strlen("Boot system")) == 0){
			bootsel->system = (char *)value;
//			printf("bootsystem = %s\n",bootsel->system);
		}
		if(strncmp(name,"Clear data", strlen("Clear data")) == 0){
			bootsel->data = (char *)value;
//			printf("Clear data = %s\n",bootsel->data);
		}
		if(strncmp(name,"Clear cache", strlen("Clear cache")) == 0){
			bootsel->cache = (char *)value;
//			printf("Clear cache = %s\n",bootsel->cache);
		}
		if(strncmp(name,"OTA device",strlen("OTA device")) == 0) {
			bootsel->device = (char *)value;		
//			printf("OTA device = %s\n",bootsel->device)
		}
