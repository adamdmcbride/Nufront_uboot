...
void scsi_scan(int mode)
{
	...
	for(i=0;i<CONFIG_SYS_SCSI_MAX_SCSI_ID;i++) {
		...
		for(lun=0;lun<CONFIG_SYS_SCSI_MAX_LUN;lun++) {
			...
			capacity=((cpu_to_le32((unsigned int)tempbuff[3]))<<24)|
					((cpu_to_le32((unsigned int)tempbuff[2]))<<16)|
					((cpu_to_le32((unsigned int)tempbuff[1]))<<8)|
					(cpu_to_le32((unsigned int)tempbuff[0]));

			blksz=((cpu_to_le32((unsigned long)tempbuff[4]))<<24)|
				((cpu_to_le32((unsigned long)tempbuff[5]))<<16)|
				((cpu_to_le32((unsigned long)tempbuff[6]))<<8)|
				(cpu_to_le32((unsigned long)tempbuff[7]));
